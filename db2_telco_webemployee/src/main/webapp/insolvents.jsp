    ...
    <div class="column is-5 packages">
        <nav class="breadcrumb" aria-label="breadcrumbs">
            <ul>
                ...
                <li class="is-active"><a href="#" aria-current="page">Insolvent Users</a></li>
            </ul>
        </nav>
        <h3 class="title is-3">
            Insolvent Users
        ...
        <table class="table">
            <thead>
                ...
                <td>
                    <strong>Suspended Orders (click to view details)</strong>
                </td>
            </thead>
            <tbody>
                <c:forEach var="insolventUser" items="${insolventUsers}">
                    <tr>
                        <td>
                            ${insolventUser.getUsername()} - ${insolventUser.getEmail()}
                        </td>
                        <td>
                            <ul>
                            <c:forEach var="order" items="${insolventUser.getOrders()}">
                                <c:if test="${not order.isValid()}">
                                    <li>
                                        <span class="tag is-info is-light js-modal-trigger" style="cursor: pointer" data-target="modal-js-order-${order.getID()}">Order #${order.getID()}</span>
                                        <div id="modal-js-order-${order.getID()}" class="modal">
                                            <div class="modal-background"></div>
                                            <div class="modal-card">
                                                <header class="modal-card-head">
                                                    <p class="modal-card-title">Order ${order.getID()}</p>
